    ...
    <div class="row">
        ...
        <div class="col d-flex justify-content-center ">
            <div class="container mt-4 pb-5 mx-auto">
                ...
                <a href="#" class="active fs-5 text-white text-decoration-none ms-2 pt-1 d-block"><img src="{{ asset('img/settings.svg') }}" class="img-fluid" alt=""> Review Management > <span class="fw-semibold">Edit Review</span></a>
                </div>

                <div class="card shadow w-75 my-4 p-4 mx-auto">
                    <form action="{{ route('update_review', $data->id) }}" method="POST" enctype="multipart/form-data" class="fs-5" id="reviewForm">
                        @csrf
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control fs-5 border-dark" id="image" name="image" accept="image/*">
                                    <img src="/profiles/{{$data->image}}" alt="Current Image" class="mt-2" width="150">
                            @error('image')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control fs-5 border-dark" id="name" name="name" placeholder="Name" value="{{$data->name}}">
                            @error('name')
                            <span class="text-danger fw-semibold">Error message goes here</span>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <textarea class="form-control fs-5 border-dark" id="description" name="description" rows="2" placeholder="Description">{{ $data->description }}</textarea>
                            @error('description')
                            <span class="text-danger fw-semibold">Error message goes here</span>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <textarea class="form-control fs-5 border-dark" id="review" name="review" rows="3" placeholder="Review">{{ $data->review }}</textarea>
                            @error('review')
                            <span class="text-danger fw-semibold">Error message goes here</span>
                            @enderror
                        </div>
...
@endsection
@section('scripts')
<script src="{{ asset('js/edit_review.js') }}"></script>
@if (session('message'))
    <script>
